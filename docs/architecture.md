...
RELAY is a durable queue plus the discipline around it. `lib/loop-queue.cjs` holds the state on disk — a backlog, exactly one in-flight task, and history — and its core call returns only `wait` (a task is running; its completion will wake you), `launch` (start this unit now), or `stop` (the queue is empty even after a refill). There is deliberately no fourth outcome, so the loop can never quietly sit on a timer. `hooks/stop-hook.cjs` shows how to wire that into an agent harness: on stop, consult the backlog and continue if work remains. The scheduled-wake cadence documented in `docs/` is a long-interval safety net that fires only if a completion event was somehow missed.
